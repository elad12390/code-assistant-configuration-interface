...
import { claudeFolderExists, backupClaudeFolder, applyConfiguration, listBackups, restoreBackup } from '../../src/manager';
...
describe('Configuration Manager', () => {
  ...
  test('should check if .claude folder exists', async () => {
  ...
  test('should create backup of .claude folder', async () => {
    ...
    await expect(fs.promises.access(backupTestFilePath, fs.constants.F_OK)).resolves.toBeUndefined();
    
    const content = await fs.promises.readFile(backupTestFilePath, 'utf8');
    expect(content).toBe('test content');
  });

  test('should apply configuration to .claude folder', async () => {
