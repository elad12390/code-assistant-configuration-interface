 ...
 */
export async function listIterations(projectDir: string): Promise<{ id: string; timestamp: string }[]> {
 ...
 */
export async function getIterationDetails(projectDir: string, iterationId: string): Promise<Iteration> {
  const iterationFilePath = path.join(projectDir, '.configurator', 'iterations', `${iterationId}.json`);
  ...
  try {
    ...
    throw new Error(`Failed to retrieve iteration ${iterationId}: ${error instanceof Error ? error.message : String(error)}`);
  }
}
...
export async function compareIterations(
  ...
  const calculateDifferences = (arr1: string[], arr2: string[]): { added: string[]; removed: string[]; unchanged: string[] } => {
  ...
  const agentsDiff = calculateDifferences(iteration1.selectedComponents.agents, iteration2.selectedComponents.agents);
  const commandsDiff = calculateDifferences(iteration1.selectedComponents.commands, iteration2.selectedComponents.commands);
  const hooksDiff = calculateDifferences(iteration1.selectedComponents.hooks, iteration2.selectedComponents.hooks);
  const mcpsDiff = calculateDifferences(iteration1.selectedComponents.mcps, iteration2.selectedComponents.mcps);
