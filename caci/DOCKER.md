...
```bash
# Show help
docker run --rm caci --help

# Show version
docker run --rm caci --version
...
- The container runs as a non-root user (`caci:nodejs`)
...
docker buildx build --platform linux/amd64,linux/arm64 -t caci .
```
